    ...
    init() {
      ...
      for(let i = 0; i < this.hotData.length; i++) {
        for(let j = 0; j < this.hotData[i].length; j++) {
          ...
          this.mesh[i * this.hotData[i].length + j] = new THREE.Mesh(geometry, material);
          // this.mesh
          // 将网格放入场景中
    this.mesh[i * this.hotData[i].length + j].x = this.hotData[i][j].x
    this.mesh[i * this.hotData[i].length + j].y = this.hotData[i][j].y
    this.mesh[i * this.hotData[i].length + j].z = this.hotData[i][j].z
          this.scene.add(this.mesh[i * this.hotData[i].length + j]);
